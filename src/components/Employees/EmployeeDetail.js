...
import { employeesDetails, Addtask, taskList } from '../../redux/action/Employees';
...
const EmployeeDetail = ({ employeesDetails, Addtask, taskList }) => {
  ...
  const EmployeeDetail = () => {
    ...
    employeesDetails(data).then((res) => {
      if(res.data.user.tasks.length===0){
        setEmpty(true)
      }
      if (res.data.status) {
        console.log(res);
        setData(res.data.user)
        setRows(res.data.user.tasks)
      } else {
        console.log(res);
      }
    })
  ...
  const handleSubmit = (value) => {
    Object.assign(value, { status: "pending", assigned_by: localStorage.getItem("user"), _id: location.state })
    console.log(value);
